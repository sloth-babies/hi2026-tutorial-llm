...
- パスワード入力後に表示する場合は `protected: true` を付ける

現在のハンズオンColabのパスワードは、`src/pages/index.astro` の画面表示用スクリプトで `hi2026` として管理している。これは認証ではなく、静的ページ上の表示切り替えである。

URLが未確定の場合は、カードやボタンを表示せず、プレースホルダーURLもコミットしない。

### 資料PDF

現在の公開ファイルは `public/materials/HI研究会_20260916.pdf`。パスワード解除後は、新しいタブでの表示またはダウンロードを選択できる。
